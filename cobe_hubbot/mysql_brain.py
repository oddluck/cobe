...
class MySQLBrain:
    ...
    def __init__(self, brain_name, mysql_host, mysql_user, mysql_password):
        """
        Construct a brain with the specified name, in the given mysql/mariadb host.
        If no brain exists, one is created.
        """
        with trace_us("Brain.connect_us"):
            self.graph = graph = Graph(brain_name, mysql_host, mysql_user, mysql_password)
...
class Graph:
    ...
    def __init__(self, database_name, mysql_host, mysql_user, mysql_password, run_migrations=True):
        try:
        ...
        if self.is_initted():
            ...
            self._all_tokens_args = " AND ".join(["token%d_id = ?" % i for i in range(self.order)])
    ...
    def get_text_by_edge(self, edge_id):
        q = f"SELECT tokens.text, edges.has_space FROM nodes, edges, tokens WHERE edges.id = ? AND edges.prev_node = nodes.id AND nodes.{self._last_token} = tokens.id"
